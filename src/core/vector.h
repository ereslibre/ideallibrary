...
#include <core/interfaces/iterator.h>
#include <core/interfaces/const_iterator.h>
...
public:
    ...
    /**
      * Prepends element @p t to the vector.
      */
    Vector<T> &operator>>(const T &t);
...
////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    /**
      ...
      */
    class ConstIterator
        : public IdealCore::ConstIterator<T>
    {
    public:
        /**
...
void Vector<T>::append(const T &t)
{
    ...
    if (!d->m_size) {
        ...
        d->m_begin = Private::m_initialContainerSize / 2;
        d->m_end = d->m_begin;
        d->m_containerSize = Private::m_initialContainerSize;
    } else if (d->m_end == d->m_containerSize || d->m_size > (0.4 * d->m_containerSize)) {
        const size_t oldBegin = d->m_begin;
        d->m_begin = d->m_containerSize - d->m_size / 2;
        d->m_end = d->m_begin + d->m_size; // Appending an item. Not substracting 1
        d->m_containerSize *= 2;
        d->m_vector = (typename Private::Element**) realloc(d->m_vector, d->m_containerSize * sizeof(typename Private::Element*));
        memmove(&d->m_vector[d->m_begin], &d->m_vector[oldBegin], d->m_size * sizeof(typename Private::Element*));
        memset(&d->m_vector[d->m_end + 1], '\0', (d->m_containerSize - (d->m_end + 1)) * sizeof(typename Private::Element*));
    } else {
        ++d->m_end;
    }
    typename Private::Element *e = new typename Private::Element;
    e->m_element = t;
    d->m_vector[d->m_end] = e;
    ++d->m_size;
    ++d->m_count;
}
...
void Vector<T>::prepend(const T &t)
{
    ...
    if (!d->m_size) {
        ...
        d->m_end = d->m_begin;
        d->m_containerSize = Private::m_initialContainerSize;
    } else if (!d->m_begin || d->m_size > (0.4 * d->m_containerSize)) {
        const size_t oldBegin = d->m_begin;
        d->m_begin = d->m_containerSize - d->m_size / 2 - 1; // Prepending an item. Substracting 1
        d->m_end = d->m_begin + d->m_size;
        d->m_containerSize *= 2;
        d->m_vector = (typename Private::Element**) realloc(d->m_vector, d->m_containerSize * sizeof(typename Private::Element*));
        memmove(&d->m_vector[d->m_begin + 1], &d->m_vector[oldBegin], d->m_size * sizeof(typename Private::Element*));
        memset(&d->m_vector[d->m_end + 1], '\0', (d->m_containerSize - (d->m_end + 1)) * sizeof(typename Private::Element*));
    } else {
        --d->m_begin;
    }
...
void Vector<T>::insertAt(const T &t, size_t i)
{
    ...
    if (i >= d->m_containerSize) {
        ...
        const size_t oldBegin = d->m_begin;
        d->m_begin = d->m_containerSize / 2 - d->m_size / 2;
        d->m_end = d->m_begin + d->m_size - (d->m_size ? 1 : 0);
        if (d->m_vector) {
            d->m_vector = (typename Private::Element**) realloc(d->m_vector, d->m_containerSize * sizeof(typename Private::Element*));
            memmove(&d->m_vector[d->m_begin], &d->m_vector[oldBegin], d->m_size * sizeof(typename Private::Element*));
            memset(&d->m_vector[d->m_end + 1], '\0', (d->m_containerSize - (d->m_end + 1)) * sizeof(typename Private::Element*));
    ...
    } else if (d->m_size > (0.4 * d->m_containerSize)) {
        const size_t oldBegin = d->m_begin;
        d->m_begin = d->m_containerSize - d->m_size / 2;
        d->m_end = d->m_begin + d->m_size - 1;
        d->m_containerSize *= 2;
        d->m_vector = (typename Private::Element**) realloc(d->m_vector, d->m_containerSize * sizeof(typename Private::Element*));
        memmove(&d->m_vector[d->m_begin], &d->m_vector[oldBegin], d->m_size * sizeof(typename Private::Element*));
        memset(&d->m_vector[d->m_end + 1], '\0', (d->m_containerSize - (d->m_end + 1)) * sizeof(typename Private::Element*));
        memmove(&d->m_vector[i + d->m_begin + 1], &d->m_vector[i + d->m_begin], (d->m_size - i) * sizeof(typename Private::Element*));
    } else {
        ++d->m_end;
        memmove(&d->m_vector[i + d->m_begin + 1], &d->m_vector[i + d->m_begin], (d->m_size - i) * sizeof(typename Private::Element*));
    }
...
void Vector<T>::removeAt(size_t i)
{
    ...
    d->copyAndDetach(this);
    if (d->m_containerSize > Private::m_initialContainerSize && d->m_size < (0.3 * d->m_containerSize)) {
        const size_t oldBegin = d->m_begin;
        d->m_containerSize /= 2;
        d->m_begin = d->m_containerSize / 2 - d->m_size / 2;
        d->m_end = d->m_begin + d->m_size - 2; // Removed an item. Update end
        memmove(&d->m_vector[d->m_begin], &d->m_vector[oldBegin], d->m_size * sizeof(typename Private::Element*));
        d->m_vector = (typename Private::Element**) realloc(d->m_vector, d->m_containerSize * sizeof(typename Private::Element*));
        if (i != d->m_size - 1) {
            memmove(&d->m_vector[i + d->m_begin], &d->m_vector[i + d->m_begin + 1], (d->m_size - i - 1) * sizeof(typename Private::Element*));
        } else {
            d->m_vector[i + d->m_begin] = 0;
        }
    } else {
        --d->m_end;
        if (i != d->m_size - 1) {
...
{
    if (i >= d->m_size) {
        IDEAL_DEBUG_WARNING("index out of range (" << i << ")");
        d->m_emptyRes = T();
        return d->m_emptyRes;
    } else if (!d->m_vector[i + d->m_begin]) {
        d->m_emptyRes = T();
        return d->m_emptyRes;
    }
    d->copyAndDetach(this);
    return d->m_vector[i + d->m_begin]->m_element;
}
